...
for (const auto& [key, value] : localParams) {
		if (key == "setBendNormalizationAngle" || key == "bendNormalizationAngle") {
			...
			if (value == "/^	enum class PostProcessingMode {$/") {
			  layout.setPostProcessing(NodeRespecterLayout::PostProcessingMode::/^	enum class PostProcessingMode {$/);
			}
 else { 
std::cout << "Bad enum value: " << value << std::endl;
std::cout << helpText << std::endl;
